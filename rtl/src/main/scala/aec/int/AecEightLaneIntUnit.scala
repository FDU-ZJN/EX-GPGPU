...
class AecEightLaneIntUnit(val lanesPerCycle: Int = 16) extends Module {
  ...
  for (g <- 0 until groups; i <- 0 until lanesPerCycle) {
    ...
    resultBanks(architecturalLane).io.writePredicate := lanes(i).io.resp.bits.predicateResult
    resultBanks(architecturalLane).io.writeError := lanes(i).io.resp.bits.error
  }
  ...
  when (io.req.fire) {
    ...
    group := 0.U
    writeMask := 0.U
    state := Mux(io.req.bits.activeMask.orR, "b0000001000".U, "b1000000000".U)
  }
