...
/** Four-entry, in-order retirement queue for one resident warp.
  ...
  */
class AecWarpRob(warpId: Int, depth: Int = 4) extends Module {
  require(warpId >= 0 && warpId < AecFrontendConfig.Warps)
  ...
  val instruction = Reg(Vec(depth, UInt(128.W)))
  ...
  val result = Reg(Vec(depth, Vec(32, UInt(64.W))))
  ...
  val completionMatches = io.completion.valid &&
    io.completion.bits.tag.warp === warpId.U &&
  ...
  io.head.bits.tag.warp := warpId.U
  ...
  io.head.bits.instruction := instruction(headPtr)
  ...
  io.head.bits.data := result(headPtr)
  ...
  when (doAllocate) {
    ...
    instruction(tailPtr) := io.allocate.bits.instruction
  ...
  when (completionMatches) {
    result(completionIndex) := io.completion.bits.data
