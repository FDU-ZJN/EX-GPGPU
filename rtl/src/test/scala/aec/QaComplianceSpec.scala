...
class QaComplianceSpec extends AnyFlatSpec with ChiselScalatestTester {
  ...
  it should "hold every request payload stable while the service applies backpressure" in {
    test(new AecExternalMemoryEngine) { dut =>
      ...
      dut.io.lineIn.bits.cacheable.poke(false.B)
      dut.io.lineIn.bits.token.poke(7.U)
      ...
      dut.io.lineComplete.bits.address.expect("h12345680".U)
      dut.io.lineComplete.bits.token.expect(7.U)
      dut.clock.step()
      dut.io.outstanding.expect(0.U)
    }
  }

  it should "reserve distinct tags while accepting one line request per cycle" in {
    test(new AecExternalMemoryEngine) { dut =>
      dut.io.lineIn.valid.poke(false.B)
      dut.io.lineComplete.ready.poke(true.B)
      dut.io.memReqReady.poke(true.B)
      dut.io.memRspValid.poke(false.B)
      dut.io.memRspTag.poke(0.U); dut.io.memRspRdata.poke(0.U); dut.io.memRspError.poke(false.B)
      dut.io.lineIn.bits.space.poke(false.B); dut.io.lineIn.bits.warp.poke(0.U)
      dut.io.lineIn.bits.write.poke(false.B); dut.io.lineIn.bits.wdata.poke(0.U)
      dut.io.lineIn.bits.wstrb.poke(0.U); dut.io.lineIn.bits.lastForInstruction.poke(false.B)
      dut.io.lineIn.bits.cacheable.poke(true.B)

      for (i <- 0 until 4) {
        dut.io.lineIn.bits.address.poke((i * 128).U)
        dut.io.lineIn.bits.token.poke(i.U)
        dut.io.lineIn.valid.poke(true.B)
        dut.io.lineIn.ready.expect(true.B)
        if (i > 0) {
          dut.io.memReqValid.expect(true.B)
          dut.io.memReqAddr.expect(((i - 1) * 128).U)
          dut.io.memReqTag.expect((i - 1).U)
        }
        dut.clock.step()
      }
      dut.io.lineIn.valid.poke(false.B)
      dut.io.memReqValid.expect(true.B)
      dut.io.memReqAddr.expect((3 * 128).U)
      dut.io.memReqTag.expect(3.U)
      dut.clock.step()
      dut.io.outstanding.expect(4.U)

      for (i <- 0 until 4) {
        dut.io.memRspTag.poke(i.U)
        dut.io.memRspValid.poke(true.B)
        dut.io.lineComplete.valid.expect(true.B)
        dut.io.lineComplete.bits.address.expect((i * 128).U)
        dut.io.lineComplete.bits.token.expect(i.U)
        dut.clock.step()
      }
      dut.io.memRspValid.poke(false.B)
      dut.io.outstanding.expect(0.U)
  ...
  it should "map adjacent LMEM lanes to distinct 4 KiB regions and reject thread-local overflow" in {
    test(new AecGmemPipeline(2)) { dut =>
      ...
      def completeRead(token: BigInt): Unit = {
        ...
        dut.io.lineComplete.bits.error.poke(false.B); dut.io.lineComplete.bits.address.poke(0.U)
        dut.io.lineComplete.bits.cacheable.poke(true.B); dut.io.lineComplete.bits.token.poke(token.U)
        dut.io.lineComplete.valid.poke(true.B)
      ...
      def waitForRequest(): Unit = {
        var cycles = 0
        while (!dut.io.lineOut.valid.peek().litToBoolean && cycles < 80) {
          dut.clock.step(); cycles += 1
        }
        assert(cycles < 80, "timed out waiting for LMEM line request")
      }
      def waitForCompletion(): Unit = {
        var cycles = 0
        while (!dut.io.done.valid.peek().litToBoolean && cycles < 80) {
          dut.clock.step(); cycles += 1
        }
        assert(cycles < 80, "timed out waiting for LMEM completion")
      }
      ...
      dut.io.lineOut.bits.address.expect((62 * 4096).U)
      val firstToken = dut.io.lineOut.bits.token.peek().litValue
      dut.clock.step()
      // The second independent line is already presented before the first
      // response, proving that the load path is no longer wait-response serial.
      ...
      completeRead(firstToken)
      completeRead(secondToken)
      waitForCompletion()
  ...
  private def initializeAtomic(dut: AecGmemPipeline, op: Int, signed: Boolean,
  ...
  private def respond(dut: AecGmemPipeline, line: BigInt, write: Boolean = false): Unit = {
    ...
    dut.io.lineComplete.bits.error.poke(false.B); dut.io.lineComplete.bits.address.poke(0.U)
    dut.io.lineComplete.bits.cacheable.poke(false.B); dut.io.lineComplete.bits.token.poke(0.U)
    dut.io.lineComplete.valid.poke(true.B)
    dut.clock.step(); dut.io.lineComplete.valid.poke(false.B)
  }

  private def waitForLineRequest(dut: AecGmemPipeline, limit: Int = 80): Unit = {
    var cycles = 0
  ...
  private def waitForDone(dut: AecGmemPipeline, limit: Int = 80): Unit = {
    var cycles = 0
  ...
  it should "encode MAX as 1 and MIN as 2 for signed and unsigned words" in {
    def run(op: Int, signed: Boolean): (Long, Long) = {
      ...
      test(new AecGmemPipeline(2)) { dut =>
        ...
        waitForLineRequest(dut); dut.clock.step(); respond(dut, 0) // preflight
        waitForLineRequest(dut); dut.clock.step()
  ...
  it should "suppress failed CAS writes and strobe only successful lanes" in {
    test(new AecGmemPipeline(2)) { dut =>
      ...
      def begin(compare0: Long, compare1: Long): Unit = {
        ...
        dut.io.start.valid.poke(true.B); dut.clock.step(); dut.io.start.valid.poke(false.B)
        waitForLineRequest(dut); dut.clock.step(); respond(dut, 2L | (3L << 32)) // preflight
        waitForLineRequest(dut); dut.clock.step(); respond(dut, 2L | (3L << 32)) // RMW read
      }
  ...
  it should "apply same-word atomic lanes in ascending lane order" in {
    test(new AecGmemPipeline(2)) { dut =>
      ...
      dut.io.start.valid.poke(true.B); dut.clock.step(); dut.io.start.valid.poke(false.B)
      waitForLineRequest(dut); dut.clock.step(); respond(dut, 5) // preflight
      waitForLineRequest(dut); dut.clock.step(); respond(dut, 5) // RMW read
      waitForLineRequest(dut)
