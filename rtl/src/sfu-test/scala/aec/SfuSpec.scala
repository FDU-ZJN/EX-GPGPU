...
import aec.fp.{AESSFU, AecQ34ToF32, AecSfuWarpUnit}
...
class SfuSpec extends AnyFlatSpec with ChiselScalatestTester with Matchers {
  ...
  it should "execute scalar FP divide, reciprocal, sqrt and transcendental operations" in {
    test(new AESSFU) { dut =>
      ...
      def run(op: UInt, a: Float, b: Float = 0.0f, dtype: Int = 8): Long = {
        ...
        dut.io.req.valid.poke(false.B)
        var cycles = 0
        while (!dut.io.resp.valid.peek().litToBoolean && cycles < 1200) { dut.clock.step(); cycles += 1 }
  ...
  behavior of "AecQ34ToF32"

  it should "normalize, round and preserve scaled Q34 values" in {
    test(new AecQ34ToF32) { dut =>
      dut.clock.setTimeout(1000)
      dut.io.req.valid.poke(false.B)
      dut.io.resp.ready.poke(true.B)

      def pack(value: BigInt, scale: Int = 0): Long = {
        while (!dut.io.req.ready.peek().litToBoolean) dut.clock.step()
        dut.io.req.bits.value.poke(value.S(48.W))
        dut.io.req.bits.scale.poke(scale.S(12.W))
        dut.io.req.valid.poke(true.B)
        dut.clock.step()
        dut.io.req.valid.poke(false.B)
        var cycles = 0
        while (!dut.io.resp.valid.peek().litToBoolean && cycles < 100) { dut.clock.step(); cycles += 1 }
        cycles should be < 100
        val bits = dut.io.resp.bits.peek().litValue.longValue & 0xffffffffL
        dut.clock.step()
        bits
      }

      val q34 = BigInt(1) << 34
      pack(q34) shouldBe 0x3f800000L
      pack(-q34) shouldBe 0xbf800000L
      pack(q34, 3) shouldBe 0x41000000L
      pack(0) shouldBe 0L
    }
  }
