...
object AecFrontendConfig {
  ...
  final val Schedulers = 1; final val ResidentWarps = 2; final val ImemWords = 1024; final val LocalWords = 16384
...
/** Full backend payload before it is written into the partition-local result
  * store.  This bundle never crosses the ROB boundary. */
class AecBackendCompletion extends Bundle {
  val tag = new AecInstructionTag
...
class AecTaggedLocalMemoryRequest extends Bundle {
  val tag = new AecInstructionTag
  val request = new AecLocalMemoryRequest
}

class AecCoordinateRequest extends Bundle {
  val special = UInt(16.W)
  val linearLane = UInt(8.W)
  val startLane = UInt(5.W)
}

class AecCoordinateResponse extends Bundle {
  val lane = UInt(5.W)
  val data = UInt(32.W)
  val last = Bool()
}

class AecLaunchRequest extends Bundle {
  val gridX = UInt(32.W); val gridY = UInt(32.W); val gridZ = UInt(32.W)
  val blockX = UInt(32.W); val blockY = UInt(32.W); val blockZ = UInt(32.W)
  val programInstructions = UInt(32.W)
  val loadError = Bool()
}

class AecLaunchResult extends Bundle {
  val accepted = Bool()
  val threads = UInt(32.W)
  val gridX = UInt(32.W); val gridY = UInt(32.W); val gridZ = UInt(32.W)
  val blockX = UInt(32.W); val blockY = UInt(32.W); val blockZ = UInt(32.W)
}
