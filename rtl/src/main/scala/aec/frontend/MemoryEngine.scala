...
class AecExternalMemoryEngine extends Module {
  val io = IO(new Bundle {
    ...
    val lineComplete = Decoupled(new Bundle { val space = Bool(); val warp = UInt(3.W); val tag = UInt(4.W); val write = Bool(); val lastForInstruction = Bool(); val rdata = UInt(1024.W); val error = Bool() })
  ...
  val holdValid = RegInit(false.B); val held = Reg(new AecLineRequest); val heldTag = Reg(UInt(4.W))
  io.lineIn.ready := !holdValid && hasFree
  when (io.lineIn.fire) { held := io.lineIn.bits; heldTag := tag; holdValid := true.B }
  io.memReqValid := holdValid
  io.memReqSpace := held.space; io.memReqWrite := held.write; io.memReqAddr := held.address
  io.memReqWdata := held.wdata; io.memReqWstrb := held.wstrb; io.memReqTag := heldTag
  when (io.memReqValid && io.memReqReady) {
    valid(heldTag) := true.B; space(heldTag) := held.space; warp(heldTag) := held.warp
    write(heldTag) := held.write; last(heldTag) := held.lastForInstruction; holdValid := false.B
  }
  ...
  io.outstanding := PopCount(valid) + holdValid
