...
source_hash=${PPA_SOURCE_HASH:-unknown}
...
if [[ -z "$asap7_root" ]]; then
  ...
  "$yosys_bin" -p "read_verilog -lib $root/sv/asap7_sram/*.v; read_verilog -sv $rtl_sources; hierarchy -check -top $top; chformal -remove; proc; memory_map; flatten; techmap; opt; tee -o $out/yosys_area.txt stat -top $top -hierarchy -tech cmos; write_verilog -noattr $out/generic_netlist.v" \
...
if ! (cd "$asap7_sram_root" && sha256sum -c "$root/../TrackB-b-013/sram/ASAP7_SRAM_LOCK.sha256"); then
...
frontend_script="read_liberty -lib $lib/AO.lib; read_liberty -lib $lib/INVBUF.lib; read_liberty -lib $lib/OA.lib; read_liberty -lib $lib/SEQ.lib; read_liberty -lib $lib/SIMPLE.lib; $sram_read_cmd read_verilog -sv $rtl_sources; hierarchy -check -top $top; chformal -remove; proc; memory_map; $flatten_cmd techmap; opt; dfflibmap -liberty $lib/SEQ.lib; opt; tee -o $out/abc_modules.txt select -list-mod *; write_rtlil $out/pre_abc.il"
mapping_failed=0
...
proc report_path_group {path_file from_set to_set path_count} {
  if {[llength \$from_set] == 0 || [llength \$to_set] == 0} {
