...
object Generate extends App {
  ...
  // Preserve the full II=1 scalar configurations as independent PPA tops.
  (new ChiselStage).emitSystemVerilog(new AecFp32PipeUnit(false), Array("--target-dir", "sv/generated/fp32_lane"))
  (new ChiselStage).emitSystemVerilog(new AecFp64PipeUnit(false, false), Array("--target-dir", "sv/generated/fp64_lane"))
}

/** Standalone generation entry point for lane-local INT PPA iteration. */
object GenerateInt extends App {
  (new ChiselStage).emitSystemVerilog(new AecEightLaneIntUnit, Array("--target-dir", "sv/generated/int"))
}
