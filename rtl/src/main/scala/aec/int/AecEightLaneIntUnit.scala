...
/**
  ...
  */
class AecEightLaneIntUnit(val lanesPerCycle: Int = 16,
  ...
  val armCapture = if (useInternalRequestBuffer) stateIdle && io.req.valid else false.B
  if (useInternalRequestBuffer) {
  ...
  io.req.ready := (if (useInternalRequestBuffer) stateCapture else stateIdle)
  ...
  for (i <- 0 until lanesPerCycle) {
    ...
    for (g <- 0 until groups) {
      ...
      requestStages(i).io.data(g).op := held.op
