...
class AecLocalLsu extends Module {
  ...
  val captureLaneEnable = RegInit(VecInit(Seq.fill(32)(true.B)))
  dontTouch(captureLaneEnable)
  ...
  for (laneIndex <- 0 until 32) {
    val preloadCompletes = state === preloadWrite && preloadByte === 15.U
    captureLaneEnable(laneIndex) := Mux(
      state === idle && !io.start.fire || io.done.fire || preloadCompletes,
      true.B,
      lane === laneIndex.U)
    when (state === idle && captureLaneEnable(laneIndex)) {
      held.address(laneIndex) := io.start.bits.address(laneIndex)
      held.storeData(laneIndex) := io.start.bits.storeData(laneIndex)
      held.compareData(laneIndex) := io.start.bits.compareData(laneIndex)
      data(laneIndex) := 0.U
    }
  }

  when (io.start.fire) {
    held.warp := io.start.bits.warp
    held.space := io.start.bits.space
    held.load := io.start.bits.load
    held.width64 := io.start.bits.width64
    held.mask := io.start.bits.mask
    held.atomic := io.start.bits.atomic
    held.atomicOp := io.start.bits.atomicOp
    held.signed := io.start.bits.signed
    lane := PriorityEncoder(io.start.bits.mask); bytePart := 0.U
    failed := false.B
    ...
    state := Mux(io.start.bits.mask.orR,
      Mux(io.start.bits.load && !io.start.bits.atomic, check, precheck), finish)
  }
  when (state === precheck) {
