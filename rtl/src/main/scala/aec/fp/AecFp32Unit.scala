...
/**
  ...
  * AecFp64Unit.
  */
class AecFp32PipeUnit(val warpIntegrated: Boolean = false) extends Module {
  val io = IO(new Bundle {
    val req = Flipped(Decoupled(new AecFpRequest))
    val resp = Decoupled(new AecFpResponse)
  })

  val pipe = Module(new YunSuanFmaPipe(2, if (warpIntegrated) 1 else 8))

  val reqPipe = io.req.bits.op === AecFpOp.add || io.req.bits.op === AecFpOp.sub ||
    io.req.bits.op === AecFpOp.mul || io.req.bits.op === AecFpOp.mad ||
    io.req.bits.op === AecFpOp.fma
  ...
  if (warpIntegrated) {
    // The warp wrapper does not issue the next lane group until every physical
    // pipe has returned.  There can therefore be only one live request here;
    // an issue-order ROB and sequence tags provide no observable behavior.
    val busy = RegInit(false.B)
    val pipePending = RegInit(false.B)
    pipeSeq := 0.U
    held := io.req.bits
    pipe.io.req.valid := io.req.valid && reqPipe && !busy
    io.req.ready := !busy && Mux(reqPipe, pipe.io.req.ready, true.B)
    pipeHead := busy && pipePending && pipe.io.resp.valid
    oldHead := busy && !pipePending
    ...
    // The standalone lane retains full II=1 behavior and may have mixed
    // arithmetic/simple operations complete out of order.
    ...
    pipeHead := pipe.io.resp.valid && issueQ.io.deq.valid && pipe.io.resp.bits.seq === issueQ.io.deq.bits
    oldHead := simpleQ.io.deq.valid && issueQ.io.deq.valid && simpleQ.io.deq.bits.seq === issueQ.io.deq.bits
  ...
  pipe.io.resp.ready := io.resp.ready && pipeHead
  val oldPredicate = isCmpP && cmpTrue
  ...
  if (warpIntegrated) {
    ...
    io.resp.bits.result := Mux(pipeHead, pipeResult, integratedSimpleResp.get.result)
    io.resp.bits.predicate_result := Mux(pipeHead, false.B, integratedSimpleResp.get.predicate_result)
    io.resp.bits.dest := Mux(pipeHead, pipe.io.resp.bits.dest, integratedSimpleResp.get.dest)
    io.resp.bits.error := Mux(pipeHead, false.B, integratedSimpleResp.get.error)
    io.resp.bits.exception_flags := Mux(pipeHead, pipe.io.resp.bits.fflags, integratedSimpleResp.get.exception_flags)
  } else {
    io.resp.bits.result := Mux(pipeHead, pipeResult, oldResult)
    io.resp.bits.predicate_result := Mux(pipeHead, false.B, oldPredicate)
    io.resp.bits.dest := Mux(pipeHead, pipe.io.resp.bits.dest, held.dest)
    io.resp.bits.error := Mux(pipeHead, false.B, oldError)
