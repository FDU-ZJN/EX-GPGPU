/*
 * Derived from OpenXiangShan/YunSuan commit
...
class FloatFMA(val fixedFormat: Option[Int] = None) extends Module{
  ...
  require(fixedFormat.forall(f => f == 2 || f == 3), "AEC only instantiates fixed f32/f64 YunSuan FMA formats")
  // A fixed format is deliberately a Scala elaboration parameter rather than
  // a runtime mux.  FIRRTL can therefore remove the unused exponent,
  // significand, special-case and normalization branches before judged RTL
  // is emitted; Yosys then prunes the constant upper compressor columns.
  val is_fp64                 = fixedFormat.map(_ == 3).getOrElse(false).B ||
    (fixedFormat.isEmpty.B && io.fp_format === 3.U(2.W))
  ...
  val is_fp32                 = fixedFormat.map(_ == 2).getOrElse(false).B ||
    (fixedFormat.isEmpty.B && io.fp_format === 2.U(2.W))
