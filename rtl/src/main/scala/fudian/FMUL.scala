...
class FMULToFADD_fflags extends Bundle {
  ...
  val isInv = Bool()
  val overflow = Bool()
  // The add stage must use the product sign when the product dominates.
  // Carrying only overflow was insufficient for opposite-sign FMA overflow.
  val prod_sign = Bool()
}
 ...
 */
  ...
  io.to_fadd.inter_flags.prod_sign := prod_sign
}
