...
class AecGmemLsu(activeLanes: Int = 32) extends Module {
  ...
  val initialParts = VecInit((0 until 32).flatMap { lane =>
    val first = firstByteCount(held.address(lane), heldBytes)
  ...
  val startParts = VecInit((0 until 32).flatMap { lane =>
    val first = firstByteCount(io.start.bits.address(lane), startBytes)
  ...
  val partData = for (lane <- 0 until activeLanes; part <- 0 until 2) yield {
    ...
    val offset = if (part == 0) held.address(lane)(6, 0) else 0.U(7.W)
  ...
  when (io.start.fire) {
    ...
    error := false.B
    val badAtomic = io.start.bits.atomic && VecInit((0 until activeLanes).map(i =>
  ...
  when (io.lineComplete.fire) {
    when (io.lineComplete.bits.error) {
      ...
      for (lane <- 0 until activeLanes) {
        val offset = held.address(lane)(6, 0)
        val shift = offset << 3
        ...
        updatedLine = Mux(group(lane * 2), (updatedLine & ~wordMask) | (next << shift), updatedLine)
      }
      ...
      when (held.load) {
        for (lane <- 0 until activeLanes; part <- 0 until 2) {
          ...
          val first = firstByteCount(held.address(lane), heldBytes)
          val bytes = if (part == 0) first else heldBytes - first
          val sourceOffset = if (part == 0) held.address(lane)(6, 0) else 0.U(7.W)
