...
The root `Makefile` forwards the CModel build/tests and the RTL flow.  The
generated-vector format is deliberately data-driven, so a CI job can also
provide a persistent vector file with `rtl/scripts/run_fp_diff.sh --vectors`.

## AEC contest-facing core

`src/main/scala/aec/frontend/EvalTop.scala` provides the contest-facing core:
one resident CTA with eight warps, four two-warp round-robin schedulers,
register banks, pair-aware destination scoreboarding, CTA barrier state, local
memory, and a 16-tag 128-byte external-memory engine. Integer, FP32/FP64,
conversion, collective, DIV/SFU, local LSU and GMEM/atomic execution paths are
connected to the common retirement machinery. `AecEvalTop` is adapted to the
required active-low-reset `aec_eval_top` by `sv/aec_eval_top.sv`.
